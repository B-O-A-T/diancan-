$(function () {
	...
	window.operateEvents = {
		'click .edit': function (e, value, row, index) {
            ...
            layer.open({
                ...
                title: '修改管理员信息',
                skin: 'alert-lay',
                content: 'admins_edit.html?adminId=' + row['adminId'],
                area: ['50%', '80%']
            });
        },
        ...
        'click .delete': function (e, value, row, index) {
            //询问框
            layer.confirm('确定要删除 [' + row['adminAccount'] + '] 吗?', {
                btn: ['确定', '取消'] //按钮
            }, function (index) {
                var url = "http://192.168.1.168:8080/deleteAdmin?id=" + row['adminId']+"&sessionId=" +sessionId;
    ...
    $('#adminstable').bootstrapTable({
        url: 'http://192.168.1.168:8080/getAdmin?sessionId=' +sessionId,
        method: 'post', //请求方式（*）
    ...
    $(".add-btn").on("click", function () {
        ...
        layer.open({
            ...
            title: '添加管理员',
            skin: 'alert-lay',
            content: 'admins_edit.html', //这里content是一个普通的String
            area: ['50%', '80%']
        });
    });
    ...
    /*选择复选框使删除所选按钮可点*/
    $('#adminstable').on('check.bs.table uncheck.bs.table ' +
        'check-all.bs.table uncheck-all.bs.table', function () {
        if ($(".delete-selected"))
            $(".delete-selected").prop('disabled', !$('#adminstable').bootstrapTable('getSelections').length);
    });
    /*这里获取你点击的复选框，返回所选的行，当没有选择任何行的时候返回一个空数组。*/
    function getIdSelections() {
        return $.map($('#adminstable').bootstrapTable('getSelections'), function (row) {
            return row.adminId
        });
    }
    ...
    $(".delete-selected").click(function () {
        ...
        function () {
            /*此处后台判断删除数据成功后页面变化*/
            var url = "http://192.168.1.168:8080/deleteAdmins?sessionId=" +sessionId;
            $.ajax({
            	type:"post",
            	url:url,
            	data:{"id":ids},
