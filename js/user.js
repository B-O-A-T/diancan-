	...
	window.operateEvents = {
		'click .edit': function (e, value, row, index) {
            ...
            layer.open({
                ...
                title: '修改会员信息',
                skin: 'alert-lay',
                content: 'user_edit.html?userId=' + row['userId'],
                area: ['50%', '80%']
            });
        },
        ...
        'click .delete': function (e, value, row, index) {
            //询问框
            layer.confirm('确定要删除 [' + row['userName'] + '] 吗?', {
                btn: ['确定', '取消'] //按钮
            }, function (index) {
                var url = "http://192.168.1.168:8080/deleteUser?id=" + row['userId']+"&sessionId=" +sessionId;
...
//  });
    ...
    $('#userstable').bootstrapTable({
        url: 'http://192.168.1.168:8080/getUsers?sessionId=' +sessionId,
        method: 'post', //请求方式（*）
    ...
    $('#userstable').on('check.bs.table uncheck.bs.table ' +
        ...
        if ($(".delete-selected"))
            $(".delete-selected").prop('disabled', !$('#userstable').bootstrapTable('getSelections').length);
    });
    ...
    $(".delete-selected").click(function () {
        ...
        function () {
            /*此处后台判断删除数据成功后页面变化*/
            var url = "http://192.168.1.168:8080/deleteUsers?sessionId=" +sessionId;
            $.ajax({
            	type:"post",
            	url:url,
            	data:{"id":ids},
